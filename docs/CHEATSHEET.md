...
**Сначала просто подождите 10–15 минут.** Обычно CI укладывается в 20–30 секунд, но при перебоях с выделением раннеров на стороне GitHub прогон создаётся и висит в очереди, не показываясь в интерфейсе. Со стороны это неотличимо от «Actions не работают». Так уже было 5–6 августа: прогоны появились с задержкой в четверть часа.

Признак именно этой ситуации: на вкладке Actions у прогона стоит время выполнения в минутах вместо секунд.

Если не помогло, по порядку:

1. Посмотреть прогоны именно этой ветки:
   `https://github.com/an-pit/dog-walks/actions?query=branch%3Aимя-ветки`
2. Проверить https://www.githubstatus.com — компонент Actions
3. Закрыть и сразу снова открыть PR — это заново шлёт событие `pull_request`
4. Пустой коммит — новый SHA, на который повесятся проверки заново:
   ```bash
   git commit --allow-empty -m "chore: перезапустить CI"
   git push
   ```
5. Крайний случай: Settings → Rules → Rulesets → `protect-main` → Edit → снять **Require status checks to pass**, влить, вернуть обратно

> Если снимали требование — **верните его**, как только Actions заработают. Иначе `main` остаётся без защиты, и это легко забыть.
...
## 11. Методика расчёта наблюдений

Зафиксировано в коде (`code/backend/src/analytics/`) и продублировано в интерфейсе.
...
**Медиана, а не среднее.** Одна прогулка на три часа заметно сдвигает среднее и почти не двигает медиану. Для данных с редкими длинными выбросами это принципиально.

**Будни отдельно от выходных.** В данных видна устойчивая разница. Если однотипных дней в окне меньше четырёх, берём все подряд — считать медиану по двум значениям бессмысленно.

**Минимум 14 дней.** Пока данных меньше, сравнения не показываются вовсе.

**Целевой день в выборку не входит** — день не сравнивается сам с собой.

**Формулировки описывают факт, а не оценку.** «На 40% меньше обычного», а не «мало гуляли». Медицинских порогов в приложении нет и быть не должно: интерпретация за человеком, при беспокойстве — за ветеринаром.
...
### Разрыв между прогулками

Считается только по `ended_at`. По `updated_at` считать нельзя: это момент редактирования записи, а не время прогулки — записи часто заполняются вечером за весь день сразу.
