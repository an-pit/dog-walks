...
> **Выполнено 3–6 августа 2026.** Всё описанное здесь работает: ветки и PR, защита `main`, тесты, CI, автодеплой, бэкапы в Object Storage. Документ оставлен как объяснение, *почему* всё устроено именно так — команды на каждый день лежат в [шпаргалке](CHEATSHEET.md).
>
> Из того, что появилось после: тесты фронтенда (`code/frontend/tests/`) и отдельный шаг для них в CI.
